    ...
    this.actions.pipe(
      ...
      mergeMap((taskData: TaskActions.GetAllStarted) => {
        return this.taskService
          .getTasks(taskData.boardId, taskData.columnId)
          .pipe(
            map((response) => {
  ...
  GetTask = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((taskData: TaskActions.GetStarted) => {
        return this.taskService
          .getTask(taskData.payload.columnId, taskData.payload.taskId)
          .pipe(
            map((response) => {
              this.store.dispatch(new Remove(false));
  ...
  UpdateTask = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((taskData: TaskActions.UpdateStarted) => {
        return this.taskService
          .updateTasks(taskData.payload, taskData.boardId, taskData.columnId)
          .pipe(
            map((response) => {
              this.store.dispatch(new Remove(false));
  ...
  DeleteTask = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((taskData: TaskActions.DeleteStarted) => {
        return this.taskService
          .deleteTask(taskData.boardId, taskData.columnId, taskData.taskId)
          .pipe(
            map(() => {
              this.store.dispatch(new Remove(false));
              return new TaskActions.Delete(
                taskData.boardId,
                taskData.columnId,
                taskData.taskId
              );
    ...
    this.actions.pipe(
      ...
      switchMap((taskData: TaskActions.FileAddStarted) => {
        return this.fileService.uploadFile(taskData.fileModel).pipe(
          map((response) => {
            this.store.dispatch(new Remove(false));
            return new TaskActions.FileAdded(
              new FileModel(taskData.fileModel.taskId, taskData.fileModel.file)
            );
          }),
          catchError((err) => {
            this.store.dispatch(new Remove(false));
