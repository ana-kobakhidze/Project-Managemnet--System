...
export class ColumnTasksComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getState.subscribe((state) => {
      ...
      let sorted = tasksForSort
        .filter((st) => st.columnId === this.columnId)
  ...
  getAllColumnIds() {
    ...
    this.allColumnIds.forEach((element, index) => {
      a +=
        ...
        element +
        "'" +
        (index + 1 === this.allColumnIds.length ? '' : ',');
    });
    a = a + ']';
  ...
  onDrop(event: CdkDragDrop<Task[]>) {
    ...
    let reordered = event.container.data.find(
      (item: any) => item.columnId !== this.columnId
    );
    reordered.columnId = this.columnId;
    reordered.oldColumnId = event.previousContainer.data[0].columnId;
    this.store.dispatch(
      new UpdateStarted(reordered, this.boardId, this.columnId)
    );
    this.store.dispatch(new Add(true));
  }
}
